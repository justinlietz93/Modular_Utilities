...
from typing import Protocol, runtime_checkable, Optional, Iterator
...
class SupportsStreaming(Protocol):
        ...
        stream_chat should yield ChatStreamEvent objects. Implementations should:
            - Emit zero or more delta events with finish=False
            - Emit exactly one terminal event with finish=True (may be empty delta)
            - On error, emit a single event with error set and finish=True
        """
