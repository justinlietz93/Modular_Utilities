"""CLI entry point."""
from __future__ import annotations

import argparse
import json
import sys
from dataclasses import replace
from pathlib import Path
from typing import Dict, Iterable, Optional

from ...application.run_service import RunService
from ...domain.configuration import (
    AssetOptions,
    CrawlerConfig,
    DiagramOptions,
    ExplainCardOptions,
    FeatureToggles,
    GraphOptions,
    MetricSources,
    OutputOptions,
    PrivacySettings,
    SourceOptions,
    ThresholdConfig,
    with_cli_overrides,
)


def load_config(path: Optional[Path]) -> CrawlerConfig:
    if not path:
        return CrawlerConfig()
    data = json.loads(path.read_text(encoding="utf-8"))
    config = CrawlerConfig()
    if "privacy" in data:
        privacy = data["privacy"]
        config = replace(
            config,
            privacy=PrivacySettings(
                allow_network=privacy.get("allow_network", config.privacy.allow_network),
                redact_secrets=privacy.get("redact_secrets", config.privacy.redact_secrets),
                redaction_patterns=privacy.get("redaction_patterns", config.privacy.redaction_patterns),
                retain_logs=privacy.get("retain_logs", config.privacy.retain_logs),
            ),
        )
    if "features" in data:
        features = data["features"]
        config = replace(
            config,
            features=FeatureToggles(
                enable_metrics=features.get("enable_metrics", config.features.enable_metrics),
                enable_badges=features.get("enable_badges", config.features.enable_badges),
                enable_bundles=features.get("enable_bundles", config.features.enable_bundles),
                enable_diagrams=features.get("enable_diagrams", config.features.enable_diagrams),
                enable_summary=features.get("enable_summary", config.features.enable_summary),
                enable_graph=features.get("enable_graph", config.features.enable_graph),
                enable_explain_cards=features.get(
                    "enable_explain_cards", config.features.enable_explain_cards
                ),
                enable_assets=features.get("enable_assets", config.features.enable_assets),
            ),
        )
    if "thresholds" in data:
        thresholds = data["thresholds"]
        config = replace(
            config,
            thresholds=ThresholdConfig(
                min_coverage=thresholds.get("min_coverage"),
                max_failed_tests=thresholds.get("max_failed_tests"),
                max_lint_warnings=thresholds.get("max_lint_warnings"),
                max_critical_vulnerabilities=thresholds.get("max_critical_vulnerabilities"),
            ),
        )
    if "output" in data:
        output = data["output"]
        config = replace(
            config,
            output=OutputOptions(
                base_directory=Path(output.get("base_directory", config.output.base_directory)),
                retention=output.get("retention", config.output.retention),
                manifest_name=output.get("manifest_name", config.output.manifest_name),
                summary_name=output.get("summary_name", config.output.summary_name),
            ),
        )
    if "metrics" in data:
        metrics = data["metrics"]
        config = replace(
            config,
            metrics=MetricSources(
                test_results=[Path(p) for p in metrics.get("test_results", [])],
                coverage_reports=[Path(p) for p in metrics.get("coverage_reports", [])],
                lint_reports=[Path(p) for p in metrics.get("lint_reports", [])],
                security_reports=[Path(p) for p in metrics.get("security_reports", [])],
            ),
        )
    if "graph" in data:
        graph = data["graph"]
        config = replace(
            config,
            graph=GraphOptions(
                preset=graph.get("preset", config.graph.preset),
                enable_diff=graph.get("enable_diff", config.graph.enable_diff),
                include_tests=graph.get("include_tests", config.graph.include_tests),
            ),
        )
    if "diagrams" in data:
        diagrams = data["diagrams"]
        config = replace(
            config,
            diagrams=DiagramOptions(
                presets=diagrams.get("presets", config.diagrams.presets),
                output_formats=diagrams.get("output_formats", config.diagrams.output_formats),
                concurrency=diagrams.get("concurrency", config.diagrams.concurrency),
                theme=diagrams.get("theme", config.diagrams.theme),
            ),
        )
    if "explain_cards" in data:
        cards = data["explain_cards"]
        config = replace(
            config,
            explain_cards=ExplainCardOptions(
                scopes=cards.get("scopes", config.explain_cards.scopes),
                mode=cards.get("mode", config.explain_cards.mode),
                require_review=cards.get("require_review", config.explain_cards.require_review),
                enable_local_model=cards.get(
                    "enable_local_model", config.explain_cards.enable_local_model
                ),
                local_model_path=Path(cards["local_model_path"]) if cards.get("local_model_path") else config.explain_cards.local_model_path,
            ),
        )
    if "assets" in data:
        assets = data["assets"]
        config = replace(
            config,
            assets=AssetOptions(
                document_extensions=tuple(
                    assets.get("document_extensions", config.assets.document_extensions)
                ),
                image_extensions=tuple(
                    assets.get("image_extensions", config.assets.image_extensions)
                ),
                audio_extensions=tuple(
                    assets.get("audio_extensions", config.assets.audio_extensions)
                ),
                video_extensions=tuple(
                    assets.get("video_extensions", config.assets.video_extensions)
                ),
                max_preview_chars=assets.get(
                    "max_preview_chars", config.assets.max_preview_chars
                ),
                generate_cards=assets.get(
                    "generate_cards", config.assets.generate_cards
                ),
            ),
        )
    if "seed" in data:
        config = replace(config, seed=data["seed"])
    return config


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Supercharged code crawler")
    parser.add_argument("--input", default=".", help="Input directory")
    parser.add_argument("--config", type=Path, help="Configuration JSON file")
    parser.add_argument("--include", nargs="*", default=[], help="Include glob patterns")
    parser.add_argument("--ignore", nargs="*", default=[], help="Ignore glob patterns")
    parser.add_argument("--allow-network", action="store_true", help="Enable outbound network")
    parser.add_argument("--no-metrics", action="store_true", help="Disable metrics ingestion")
    parser.add_argument("--no-badges", action="store_true", help="Disable badge generation")
    parser.add_argument("--no-bundles", action="store_true", help="Disable bundle builder")
    parser.add_argument("--no-diagrams", action="store_true", help="Disable diagram rendering")
    parser.add_argument("--no-summary", action="store_true", help="Disable summary generation")
    parser.add_argument("--no-graph", action="store_true", help="Disable knowledge graph generation")
    card_toggle_group = parser.add_mutually_exclusive_group()
    card_toggle_group.add_argument(
        "--explain-cards",
        action="store_true",
        help="Enable explain card generation",
    )
    card_toggle_group.add_argument(
        "--no-explain-cards",
        action="store_true",
        help="Disable explain card generation",
    )
    asset_toggle_group = parser.add_mutually_exclusive_group()
    asset_toggle_group.add_argument(
        "--assets",
        action="store_true",
        help="Enable non-code asset processing",
    )
    asset_toggle_group.add_argument(
        "--no-assets",
        action="store_true",
        help="Disable non-code asset processing",
    )
    parser.add_argument("--preset", default="all", help="Bundle preset to use")
    parser.add_argument("--no-incremental", action="store_true", help="Disable cache reuse")
    parser.add_argument("--force-rebuild", action="store_true", help="Force rebuild regardless of cache")
    parser.add_argument("--seed", type=int, help="Seed for deterministic operations")
    parser.add_argument("--retention", type=int, help="Retention count for runs")
    parser.add_argument("--metrics-test", nargs="*", default=[], help="JUnit XML files")
    parser.add_argument("--metrics-coverage", nargs="*", default=[], help="Coverage reports (lcov or Cobertura XML)")
    parser.add_argument("--metrics-lint", nargs="*", default=[], help="Lint SARIF reports")
    parser.add_argument("--metrics-security", nargs="*", default=[], help="Security SARIF reports")
    parser.add_argument("--min-coverage", type=float, help="Minimum coverage threshold")
    parser.add_argument("--max-failed-tests", type=int, help="Maximum failed tests")
    parser.add_argument("--max-lint-warnings", type=int, help="Maximum lint warnings")
    parser.add_argument("--max-critical-vulns", type=int, help="Maximum critical vulnerabilities")
    parser.add_argument("--graph-scope", choices=["full", "code", "dependencies", "tests"], help="Knowledge graph preset scope")
    graph_tests_group = parser.add_mutually_exclusive_group()
    graph_tests_group.add_argument("--graph-include-tests", action="store_true", help="Force include tests in graph outputs")
    graph_tests_group.add_argument("--graph-no-tests", action="store_true", help="Exclude tests from graph outputs")
    graph_diff_group = parser.add_mutually_exclusive_group()
    graph_diff_group.add_argument("--graph-diff", action="store_true", help="Force enable graph diff generation")
    graph_diff_group.add_argument("--no-graph-diff", action="store_true", help="Disable graph diff generation")
    parser.add_argument(
        "--diagram-preset",
        action="append",
        dest="diagram_presets",
        help="Diagram preset to enable (architecture, dependencies, tests).",
    )
    parser.add_argument(
        "--diagram-format",
        action="append",
        dest="diagram_formats",
        help="Diagram output format (svg, png).",
    )
    parser.add_argument(
        "--diagram-concurrency",
        type=int,
        help="Diagram rendering concurrency level",
    )
    parser.add_argument(
        "--diagram-theme",
        choices=["auto", "light", "dark"],
        help="Diagram theme selection",
    )
    parser.add_argument(
        "--card-mode",
        choices=["template", "local-llm"],
        help="Explain card generation mode",
    )
    parser.add_argument(
        "--card-scope",
        action="append",
        dest="card_scopes",
        help="Explain card scope to include (architecture, quality, tests)",
    )
    card_review_group = parser.add_mutually_exclusive_group()
    card_review_group.add_argument(
        "--card-require-review",
        action="store_true",
        help="Require manual review before explain cards are marked complete",
    )
    card_review_group.add_argument(
        "--card-auto-approve",
        action="store_true",
        help="Mark explain cards as auto-approved (no manual review)",
    )
    parser.add_argument(
        "--card-enable-local-model",
        action="store_true",
        help="Allow explain cards to use local model adapters when available",
    )
    parser.add_argument(
        "--card-local-model",
        type=Path,
        help="Path to a local model binary or weights directory for explain cards",
    )
    parser.add_argument(
        "--asset-preview",
        type=int,
        help="Maximum characters to retain in asset previews",
    )
    asset_card_group = parser.add_mutually_exclusive_group()
    asset_card_group.add_argument(
        "--asset-cards",
        action="store_true",
        help="Enable asset card generation",
    )
    asset_card_group.add_argument(
        "--no-asset-cards",
        action="store_true",
        help="Disable asset card generation",
    )
    parser.add_argument("--version", action="store_true", help="Print version and exit")
    return parser


def apply_cli(config: CrawlerConfig, args: argparse.Namespace) -> CrawlerConfig:
    config = replace(
        config,
        sources=SourceOptions(
            root=Path(args.input).resolve(),
            include=args.include or config.sources.include,
            ignore=args.ignore or config.sources.ignore,
            follow_symlinks=config.sources.follow_symlinks,
            incremental=not args.no_incremental,
        ),
        metrics=MetricSources(
            test_results=[Path(p) for p in args.metrics_test] or config.metrics.test_results,
            coverage_reports=[Path(p) for p in args.metrics_coverage] or config.metrics.coverage_reports,
            lint_reports=[Path(p) for p in args.metrics_lint] or config.metrics.lint_reports,
            security_reports=[Path(p) for p in args.metrics_security] or config.metrics.security_reports,
        ),
    )

    overrides: Dict[str, object] = {
        "allow_network": args.allow_network,
        "enable_metrics": None if not args.no_metrics else False,
        "enable_badges": None if not args.no_badges else False,
        "enable_bundles": None if not args.no_bundles else False,
        "enable_diagrams": None if not args.no_diagrams else False,
        "enable_summary": None if not args.no_summary else False,
        "enable_graph": False if args.no_graph else None,
        "enable_explain_cards": True if args.explain_cards else (False if args.no_explain_cards else None),
        "enable_assets": True if args.assets else (False if args.no_assets else None),
        "retention": args.retention,
        "incremental": None if args.force_rebuild else (not args.no_incremental),
        "force_rebuild": args.force_rebuild,
        "seed": args.seed,
        "graph_preset": args.graph_scope,
        "graph_include_tests": True if args.graph_include_tests else (False if args.graph_no_tests else None),
        "graph_enable_diff": True if args.graph_diff else (False if args.no_graph_diff else None),
        "diagram_presets": args.diagram_presets,
        "diagram_formats": [fmt.lower() for fmt in args.diagram_formats] if args.diagram_formats else None,
        "diagram_concurrency": args.diagram_concurrency,
        "diagram_theme": args.diagram_theme,
        "card_mode": args.card_mode,
        "card_scopes": [scope.lower() for scope in args.card_scopes] if args.card_scopes else None,
        "card_require_review": True if args.card_require_review else (False if args.card_auto_approve else None),
        "card_enable_local_model": args.card_enable_local_model,
        "card_model_path": args.card_local_model,
        "asset_preview_chars": args.asset_preview,
        "asset_generate_cards": True if args.asset_cards else (False if args.no_asset_cards else None),
    }

    overrides = {key: value for key, value in overrides.items() if value is not None}

    config = with_cli_overrides(config, overrides)
    thresholds = ThresholdConfig(
        min_coverage=args.min_coverage if args.min_coverage is not None else config.thresholds.min_coverage,
        max_failed_tests=args.max_failed_tests if args.max_failed_tests is not None else config.thresholds.max_failed_tests,
        max_lint_warnings=args.max_lint_warnings if args.max_lint_warnings is not None else config.thresholds.max_lint_warnings,
        max_critical_vulnerabilities=args.max_critical_vulns if args.max_critical_vulns is not None else config.thresholds.max_critical_vulnerabilities,
    )
    config = replace(config, thresholds=thresholds)
    if args.seed is not None:
        config = replace(config, seed=args.seed)
    return config


def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = build_parser()
    args = parser.parse_args(list(argv) if argv is not None else None)
    if args.version:
        print(RunService.TOOL_VERSION)
        return 0

    config = load_config(args.config)
    config = apply_cli(config, args)

    service = RunService(config)
    outcome = service.execute(preset=args.preset, incremental=config.sources.incremental)
    print(f"Run {outcome.run_id} complete. Manifest: {outcome.manifest_path}")
    if outcome.gates and not outcome.gates.passed:
        return 2
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
